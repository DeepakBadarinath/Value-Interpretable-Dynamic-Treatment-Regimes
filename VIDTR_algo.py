...
class VIDTR:
    ...
    def __init__(self, MDP, max_lengths,
        ...
        if type(stepsizes) == float or type(stepsizes) == int:
            stepsizes = np.ones((1, MDP.states.dimension))
    ...
    def maximum_over_actions(self, function):
    ...
    def bellman_function(self, t):
        ...
        def bellman_map(s,a):
            ...
            dbu_iter_class = disjoint_box_union.DBUIterator(self.MDP.states)
            dbu_iterator = iter(dbu_iter_class)
            
            
            return self.MDP.reward_functions[t](s, a, self.MDP.states, self.MDP.actions) + self.MDP.gamma * (
                    np.sum([self.MDP.transition_kernels[t](s_new, s, a, self.MDP.states, self.MDP.actions) * self.true_values[t+1](s_new) 
    ...
    def compute_optimal_policies(self):
        ...
        const_policy = lambda s: self.MDP.actions[0]
    ...
    def compute_interpretable_policies(self):
        ...
        cond_iter_class = disjoint_box_union.ConditionsIterator(self.MDP.states,
                                                                self.max_complexity)
        cond_iterator = iter(cond_iter_class)
        state_bounds = self.MDP.states.get_total_bounds()
        
        remaining_space = self.MDP.states
        ...
        for t in np.arange(self.MDP.time_horizon-1, -1, -1):
            ...
            for l in range(self.max_lengths[t]-1):
                ...
                for i in range(max_conditions):
                    ...
                    for a in self.MDP.actions:
                        ...
                        bellman_function = lambda s: self.maximum_over_actions(self.bellman_function(t))(s) -VIDTR.fix_a(self.bellman_function(t), a=a)(s)
            ...
            min_error = np.inf
            for a in self.MDP.actions:
                integrating_function = lambda s: self.maximum_over_actions(self.bellman_function(t))(s) -VIDTR.fix_a(self.bellman_function(t), a=a)(s) - self.constant_eta_function()(s,a)
